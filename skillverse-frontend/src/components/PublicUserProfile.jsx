...
const PublicUserProfile = () => {
  ...
  useEffect(() => {
    ...
    // Fetch updates
    axios.get(`http://localhost:8081/api/progress-updates/user/${userId}`)
      .then(res => setUpdates(res.data))
      .catch(() => setMessage('Failed to load user progress updates.'));
  }, [userId, navigate]);

  useEffect(() => {
    // Check follow status, only if logged in and viewing another profile
    if (currentUserId && userId && currentUserId !== userId) {
      axios.get(`http://localhost:8081/api/user-profiles/${userId}/is-following`, {
  ...
  const handleFollow = () => {
    ...
    const url = `http://localhost:8081/api/user-profiles/${userId}/follow`;
    ...
    .catch(() => setMessage('Could not update follow status.'));
  };
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-10">
          ...
          {currentUserId && currentUserId !== userId && (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-white transition 
                ${isFollowing ? 'bg-red-500 hover:bg-red-600' : 'bg-indigo-600 hover:bg-indigo-700'}`}
            >
        ...
        <section className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold mb-4 text-indigo-700">Recent Updates</h2>
                  ...
                  return (
                    <li key={u.id} className="border-l-4 border-indigo-500 pl-4">
                      <div className="flex justify-between">
                        <span className="font-medium text-indigo-600">{u.category}</span>
                        <span className="text-sm text-gray-500">
                          {new Date(u.progressDate).toLocaleDateString()}
                        </span>
                      </div>
                      <p className="mt-1 text-gray-700 whitespace-pre-wrap">{text}</p>
                    </li>
                  );
                })}
            </ul>
